        ...
        public dynamic OpticalbindingDetails(int Cmpid, string OOID)
        {
            ...
            OpticalDetailsUpdate.OpticalbindingDet = (from OOT in OpticalOrderTran.Where(x => x.RandomUniqueID == OOID && x.IsCancelled == false)
                                                      select new OpticalbindingDet
                                                      {
                                                          ...
                                                          FrameShapeID = onel.Where(v => v.OLMID == WYNKContext.Lenstrans.Where(y => y.ID == OOT.LTID).Select(x => x.FrameShapeID).FirstOrDefault()).Select(x => x.ParentDescription).FirstOrDefault() != null ? "Shape : " + onel.Where(v => v.OLMID == WYNKContext.Lenstrans.Where(y => y.ID == OOT.LTID).Select(x => x.FrameShapeID).FirstOrDefault()).Select(x => x.ParentDescription).FirstOrDefault() + "; " : null,
                                                          FrameStyleID = onel.Where(v => v.OLMID == WYNKContext.Lenstrans.Where(y => y.ID == OOT.LTID).Select(x => x.FrameStyleID).FirstOrDefault()).Select(x => x.ParentDescription).FirstOrDefault() != null ? "Style : " + onel.Where(v => v.OLMID == WYNKContext.Lenstrans.Where(y => y.ID == OOT.LTID).Select(x => x.FrameStyleID).FirstOrDefault()).Select(x => x.ParentDescription).FirstOrDefault() + "; " : null,
                                                          FrameTypeID = onel.Where(v => v.OLMID == WYNKContext.Lenstrans.Where(y => y.ID == OOT.LTID).Select(x => x.FrameTypeID).FirstOrDefault()).Select(x => x.ParentDescription).FirstOrDefault() != null ? "Type : " + onel.Where(v => v.OLMID == WYNKContext.Lenstrans.Where(y => y.ID == OOT.LTID).Select(x => x.FrameTypeID).FirstOrDefault()).Select(x => x.ParentDescription).FirstOrDefault() + "; " : null,
                                                          FrameWidthID = onel.Where(v => v.OLMID == WYNKContext.Lenstrans.Where(y => y.ID == OOT.LTID).Select(x => x.FrameWidthID).FirstOrDefault()).Select(x => x.ParentDescription).FirstOrDefault() != null ? "Width : " + onel.Where(v => v.OLMID == WYNKContext.Lenstrans.Where(y => y.ID == OOT.LTID).Select(x => x.FrameWidthID).FirstOrDefault()).Select(x => x.ParentDescription).FirstOrDefault() : null,
