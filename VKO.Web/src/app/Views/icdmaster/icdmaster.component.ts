...
export class IcdmasterComponent implements OnInit {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.success) {
        ...
        if (item.roomtype != null) {
          var RoomT = this.Rooms.find(x => x.Value == item.roomtype)
        }
  ...
  getAllDropdowns() {
    ...
    this.commonService.getListOfData('Common/GetRooms/' + parseInt(localStorage.getItem("CompanyID"))).subscribe(data => { this.Rooms = data; });
  ...
  AddIcdmappingNewgrid() {
    ...
    if (this.commonService.data.ICDExtenstionDOC.length == 0) {
      ...
      this.commonService.data.ICDExtenstionDOC.unshift(paydetails);
